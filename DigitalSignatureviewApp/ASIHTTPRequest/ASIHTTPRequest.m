...
- (BOOL)isResponseCompressed
{
	...
	return encoding && [encoding rangeOfString:@"tgz"].location != NSNotFound;
}
...
- (void)updateDownloadProgress
{
    ...
    float progressAmount = (float)(([self totalBytesRead]+[self partialDownloadSize]*1.0)/
                                   ([self contentLength]+[self partialDownloadSize]*1.0));

    self.progress = progressAmount;
    
    if (delegate && [delegate respondsToSelector:@selector(updateDownLoadProgressWithValue:)]) {
        ...
        [delegate performSelector:@selector(updateDownLoadProgressWithValue:) 
                       withObject:self];
